...
local shopProfit = Config.shopProfit['norauto']
...
function resetMenu()
	if not mainMenu and PlayerData.job then
		...
		if PlayerData.job.name == 'norauto' then
			mainMenu = RageUI.CreateMenu("", "Interaction")
			society = 'society_norauto'
		elseif PlayerData.job.name == 'norauto' then
			mainMenu = RageUI.CreateMenu("", "Interaction")
			society = 'society_norauto'
		elseif PlayerData.job.name == 'norauto' then
			mainMenu = RageUI.CreateMenu("", "Interaction")
...
function getCarPrice()
	...
	if vehPedIsIn then
		for i = 1, #Config.Vehicles, 1 do
			if GetEntityModel(vehPedIsIn) == GetHashKey(Config.Vehicles[i].model) then
				onVehList = true
				vehiclePrice = Config.Vehicles[i].price
				break
			end
		end
...
Citizen.CreateThread(function()
	while true do
		...
		if IsPedInAnyVehicle(playerPed, false) then
			...
			if (PlayerData.job and PlayerData.job.name == 'norauto' or PlayerData.job and PlayerData.job.name == 'norauto') or not Config.IsMecanoJobOnly and not lsMenuIsShowed then
				isInLSMarker = false
				for k,v in pairs(Config.Zones) do
					local dst = GetDistanceBetweenCoords(coords, v.Pos.x, v.Pos.y, v.Pos.z)
					if dst <= 5.0 then
						isInLSMarker  = true
						ESX.ShowHelpNotification(Config.Hint)
			...
			if IsControlJustReleased(0, 38) and not lsMenuIsShowed and isInLSMarker then
				if (PlayerData.job and (PlayerData.job.name == 'norauto' or PlayerData.job.name == 'norauto' or PlayerData.job.name == 'norauto')) or not Config.IsMecanoJobOnly then
					--vehPedIsIn = GetVehiclePedIsIn(playerPed, false)
					terminatePurchase()
					getCarPrice()
					vehicleClass = GetVehicleClass(vehPedIsIn)
					--print("vehicleClass: " .. vehicleClass)
					if ((vehicleClass ~= 8 or not Config.IsMotorCycleBikerOnly) and PlayerData.job.name == 'norauto') or 
						((vehicleClass ~= 8 or not Config.IsMotorCycleBikerOnly) and PlayerData.job.name == 'norauto') or
						(vehicleClass == 8 and PlayerData.job.name == 'soa') then

						--RageUI.CloseAll()
						vehModsOld = ESX.Game.GetVehicleProperties(vehPedIsIn)
						for kCat,vCat in pairs(Config.DetailsColor) do
							for kColor,vColor in pairs(Config.DetailsColor[kCat].id) do
								if vehModsOld['color1'] == Config.DetailsColor[kCat].id[kColor] then
									newPrimaryColorIndex = kCat
									newPrimaryColorDetailsIndex = kColor
								end
								if vehModsOld['color2'] == Config.DetailsColor[kCat].id[kColor] then
									newSecondaryColorIndex = kCat
									newSecondaryColorDetailsIndex = kColor
								end
								if vehModsOld['pearlescentColor'] == Config.DetailsColor[kCat].id[kColor] then
									newPearlescentColorIndex = kCat
									newPearlescentColorDetailsIndex = kColor
								end
								if vehModsOld['wheelColor'] == Config.DetailsColor[kCat].id[kColor] then
						...
						shopProfit = Config.shopProfit[PlayerData.job and PlayerData.job.name]
			...
			RageUI.IsVisible(mainMenu, function()
				if (PlayerData.job and PlayerData.job.name == 'norauto') then
					...
					RageUI.Slider("Couleur plaque", newPlateIndex, 6, "", false, {}, true, 
						{
							onSliderChange = function(Index)
								newPlateIndex = Index
								SetVehicleNumberPlateTextIndex(vehPedIsIn, Config.PlateText[newPlateIndex]["id"])
			...
			RageUI.IsVisible(colorMenu, function()
					...
					if colorPartIndex <= 3 or colorPartIndex == 5 then
						if tempColorParts['mod'][colorPartIndex] == 'primary' then
							...
							RageUI.List('Détails couleur',
								Config.DetailsColor[newPrimaryColorIndex]['label'], 
								...
								true, 
								{
									onListChange = function(Index, CurrentItems)
										if primaryColorIndex ~= Index then
											...
											primaryColorIndex = Config.DetailsColor[newPrimaryColorIndex]['id'][Index]	
											SetVehicleColours(vehPedIsIn, Config.DetailsColor[newPrimaryColorIndex]['id'][Index] , Config.DetailsColor[newSecondaryColorIndex]['id'][newSecondaryColorDetailsIndex])
						...
						elseif tempColorParts['mod'][colorPartIndex] == 'secondary' then
							...
							RageUI.List('Détails couleur',
								Config.DetailsColor[newSecondaryColorIndex]['label'], 
								...
								true, 
								{
									onListChange = function(Index, CurrentItems)
										if secondaryColorIndex ~= Index then
											...
											secondaryColorIndex = Config.DetailsColor[newSecondaryColorIndex]['id'][Index]
											SetVehicleColours(vehPedIsIn, Config.DetailsColor[newPrimaryColorIndex]['id'][newPrimaryColorDetailsIndex] ,  Config.DetailsColor[newSecondaryColorIndex]['id'][Index])
						...
						elseif tempColorParts['mod'][colorPartIndex] == 'interior' then
							...
							RageUI.List('Détails couleur',
								Config.DetailsColor[newInteriorColorIndex]['label'], 
								...
								true, 
								{
									onListChange = function(Index, CurrentItems)
										if InteriorColorIndex ~= Index then
											...
											InteriorColorIndex = Config.DetailsColor[newInteriorColorIndex]['id'][Index]
											SetVehicleInteriorColour(vehPedIsIn, Config.DetailsColor[newInteriorColorIndex]['id'][Index])
						...
						elseif tempColorParts['mod'][colorPartIndex] == 'pearlescent' then
							...
							RageUI.List('Détails couleur',
								Config.DetailsColor[newPearlescentColorIndex]['label'], 
								...
								true, 
								{
									onListChange = function(Index, CurrentItems)
										if pearlColorIndex ~= Index then
											...
											pearlColorIndex = Config.DetailsColor[newPearlescentColorIndex]['id'][Index]
											SetVehicleExtraColours(vehPedIsIn,  Config.DetailsColor[newPearlescentColorIndex]['id'][Index], Config.DetailsColor[newWheelColorIndex]['id'][newWheelColorDetailsIndex])
					...
					elseif tempColorParts['mod'][colorPartIndex] == 'wheels' then
						...
						RageUI.List('Détails couleur',
							Config.DetailsColor[newWheelColorIndex]['label'], 
							...
							true, 
							{
								onListChange = function(Index, CurrentItems)
									if pearlColorIndex ~= Index then
										...
										wheelColorIndex = Config.DetailsColor[newWheelColorIndex]['id'][Index]
										SetVehicleExtraColours(vehPedIsIn,  Config.DetailsColor[newPearlescentColorIndex]['id'][newPearlescentColorDetailsIndex], Config.DetailsColor[newWheelColorIndex]['id'][Index])
					...
					elseif tempColorParts['mod'][colorPartIndex] == 'tireSmoke' then 
						RageUI.Button(smokeColorIndex[1]['label'] .. " [" .. smokeColorIndex[1]['index'] .. "]", smokeColorIndex[1]['index'], {}, true, {
						...
						RageUI.Button(smokeColorIndex[2]['label'] .. " [" .. smokeColorIndex[2]['index'] .. "]", smokeColorIndex[2]['index'], {}, true, {
							onSelected = function(Index)
						...
						RageUI.Button(smokeColorIndex[3]['label'] .. " [" .. smokeColorIndex[3]['index'] .. "]", smokeColorIndex[3]['index'], {}, true, {
							onSelected = function(Index)
			...
			RageUI.IsVisible(neonMenu, function()
					...
					if neon1 or neon2 or neon3 or neon4 then
						RageUI.Button(neonIndex[1]['label'] .. " [" .. neonIndex[1]['index'] .. "]", neonIndex[1]['index'], {}, true, {
							onSelected = function(Index)
						...
						RageUI.Button(neonIndex[2]['label'] .. " [" .. neonIndex[2]['index'] .. "]", neonIndex[2]['index'], {}, true, {
							onSelected = function(Index)
						...
						RageUI.Button(neonIndex[3]['label'] .. " [" .. neonIndex[3]['index'] .. "]", neonIndex[3]['index'], {}, true, {
							onSelected = function(Index)
			...
			RageUI.IsVisible(cartMenu, function()
				...
				if menuItemCount == 0 then
					...
					if PlayerData.job.grade_name == 'boss' then
						...
						RageUI.Slider(_U('shopReduction') .. " (" .. shopReduction .. "%)", shopReduction, 100, "", false, {}, true, 
							{
								onSliderChange = function(Index)
						...
						RageUI.Button(_U('reductionCosts') , _U('reductionCostsTxt'), {
								LeftBadge = nil,
								RightBadge = nil,
						...
						RageUI.Button(_U('valueTotalWithReduction') , _U('valueTotalPayWithReduction'), {
								LeftBadge = nil,
								RightBadge = nil,
					...
					RageUI.Button(_U('finishPurchase') , "", {
							LeftBadge = nil,
							RightBadge = nil,
							RightLabel = nil
						}, not stop, {
...
AddEventHandler('fpwn_customs:canBill', function(amount, targetId)
	print("success")
	--TriggerServerEvent('esx_billing:sendBill', targetId, society, _U('norauto'), amount)
	terminatePurchase()
end)
